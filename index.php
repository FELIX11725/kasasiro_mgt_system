<?php
// Ensure this path is correct relative to index.php
require_once 'auth/auth.php';

// Require login for accessing this page
// If not logged in, will redirect to auth/login.php
require_login();
                        ...
                        <div class="nav-item dropdown d-none d-md-flex me-3">
                          <a href="#" class="nav-link px-0" data-bs-toggle="dropdown" tabindex="-1" aria-label="Show notifications">
                            ...
                            <?php if ($unread_notification_count > 0): ?>
                                <span class="badge bg-red"><?php echo $unread_notification_count; ?></span>
                            <?php endif; ?>
                          </a>
                              ...
                              <div class="list-group list-group-flush list-group-hoverable" id="notification-list">
                                <?php if (empty($user_notifications)): ?>
                                    <div class="list-group-item">
                                        <div class="text-secondary text-center">No new notifications.</div>
                                    </div>
                                <?php else:
                                    foreach ($user_notifications as $notification):
                                        $is_unread_class = $notification['is_read'] ? '' : 'fw-bold';
                                        $base_url = strtok($_SERVER["REQUEST_URI"],'?'); // Get current page URL without query string
                                        $mark_read_url = 'mark_notification_read.php?id=' . $notification['notification_id'] . '&return_url=' . urlencode($notification['link_url'] ?? $base_url);
                                ?>
                                <div class="list-group-item <?php echo $notification['is_read'] ? '' : 'list-group-item-primary'; ?>">
                                  <div class="row align-items-center">
                                    ...
                                    <div class="col text-truncate">
                                      <a href="<?php echo htmlspecialchars($mark_read_url); ?>" class="text-body d-block <?php echo $is_unread_class; ?> stretched-link notification-item" data-id="<?php echo $notification['notification_id']; ?>">
                                        <?php echo htmlspecialchars(substr($notification['message'], 0, 100)); ?>
                                        <?php if(strlen($notification['message']) > 100) echo "..."; ?>
                                      </a>
                                      <div class="d-block text-secondary text-truncate mt-n1">
                                        <small><?php echo htmlspecialchars(date("M j, Y, g:i a", strtotime($notification['created_at']))); ?></small>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <?php endforeach; endif; ?>
                                 <div class="list-group-item">
                                    <a href="#" class="text-secondary text-center d-block">View all notifications (Not implemented)</a>
                                 </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <?php endif; // end is_logged_in for notifications ?>
                      </div>
                          ...
                          <span class="avatar avatar-sm" style="background-image: url(./static/avatars/000f.jpg)"></span>
                           <?php if(is_logged_in() && isset($_SESSION['username'])): ?>
                            <div class="d-none d-xl-block ps-2">
                              ...
                              <div class="mt-1 small text-secondary"><?php echo htmlspecialchars(ucfirst($_SESSION['role_name'] ?? 'User')); ?></div>
                            </div>
                           <?php endif; ?>
                        </a>
